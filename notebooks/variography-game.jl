...
# ╔═╡ 4ba2a824-3afa-11f0-2240-bdcb97feb199
begin
...
# ╔═╡ 815cb82f-092d-433f-a753-e710e306dd30
md"""
# Variography
...
# ╔═╡ 540957ef-da55-4495-b150-57d7cee2d525
begin
	json = download(
...
# ╔═╡ abfa3800-cf8e-41a1-83d5-ac2d2b9e9d31
begin
	item = data[id]
...
# ╔═╡ 04af4612-4f7e-4102-95ae-032d13498c84
md"""
...
# ╔═╡ 6a420e33-f1b1-4a68-9cf2-786f79bd7bc5
md"""
The corresponding samples are obtained by looping over the pixels:
"""

# ╔═╡ 9d5318f3-4b4f-4ac1-86b4-ec0f4e3c7f2e
begin
	I = georef((X=X, Y=Y))
	S = I |> Sample(5000, replace=false)
	ρ = cor(vec(X), vec(Y))
	hscatter(S, :X, :Y,
			 axis = (
				 title = "ρ = $ρ",
...
# ╔═╡ 3f296186-6e5f-440a-82aa-04d1fac1cc8e
md"""
...
# ╔═╡ 749aed6b-d3b3-4553-8f09-ced28a7f2665
Lx, Ly = 200, 200;

# ╔═╡ 70ad44c3-9bdd-46c1-acc0-b1ff77733f13
md"""
Arrow length: $(@bind h PlutoUI.Slider(1:Lx-1, default=Lx÷2))
"""

# ╔═╡ 0e30e360-fc58-49af-b369-fc9d8de6b91e
md"""
Slide controls:
$(@bind ix PlutoUI.Slider(0:Lx-h-1, default=10))
$(@bind iy PlutoUI.Slider(0:Ly-1, default=10))
"""

# ╔═╡ 181c8641-b332-4fa9-b1df-8c9ff498669c
begin
	II = I[(1:Lx, 1:Ly), :]
	
	xi = @sprintf "%.2f" II[(ix+1,   iy+1), "X"]
	xj = @sprintf "%.2f" II[(ix+h+1, iy+1), "X"]
	yi = @sprintf "%.2f" II[(ix+1,   iy+1), "Y"]
	yj = @sprintf "%.2f" II[(ix+h+1, iy+1), "Y"]

	fig = Mke.Figure(size=(800, 400))
	ax1 = Mke.Axis(fig[1, 1], title = "(xᵢ = $xi, xⱼ = $xj)")
	viz!(ax1, II.geometry, color = II.X, colormap="grays")
	Mke.arrows!(ax1, [ix+1/2], [iy+1/2], [h], [0], color = "cyan", linewidth=3)
	Mke.text!(ax1, [ix-5], [iy-5], text = "i", color = "white", fontsize = 20)
	Mke.text!(ax1, [ix+h+5], [iy-5], text = "j", color = "white", fontsize = 20)
	ax2 = Mke.Axis(fig[1, 2], title = "(yᵢ = $yi, yⱼ = $yj)")
	viz!(ax2, II.geometry, color = II.Y, colormap="grays")
	Mke.arrows!(ax2, [ix+1/2], [iy+1/2], [h], [0], color = "cyan", linewidth=3)
	Mke.text!(ax2, [ix-5], [iy-5], text = "i", color = "white", fontsize = 20)
	Mke.text!(ax2, [ix+h+5], [iy-5], text = "j", color = "white", fontsize = 20)
	fig
end

# ╔═╡ 67041244-a5ee-43b0-a23f-7b9af5eb121f
md"""
...
# ╔═╡ 6ee31f33-a706-4efd-9580-a02c300e15d3
begin
	𝒟 = georef((X=X, Y=Y))
	𝒮 = 𝒟 |> Sample(5000, replace=false)
end;

# ╔═╡ e8801dee-ff7e-4bb6-be16-ede5bcd00370
md"""
...
# ╔═╡ 76bbee3b-e1d2-43eb-9344-98ef2c79610a
begin
	H = Symbol(hselection)
	T = Symbol(tselection)
end;
...
# ╔═╡ fcae2caf-cf03-4480-8554-13c5573b1e9d
md"""
...
# ╔═╡ 97a78d58-303d-4c8a-ac0f-94dbb906641a
let
	fig = Mke.Figure()
	ax = Mke.Axis(fig[1, 1],
				  title = "Exponential correlogram",
				  xlabel = "h",
				  ylabel = "ρ(h)"
	)
	funplot!(fig, ExponentialCovariance())
	fig
end

# ╔═╡ 0459daa1-9615-4325-acf1-d399139f6556
md"""
...
# ╔═╡ 0c56f5b3-7f06-40c1-93fb-5f6d02ebca88
let
	fig = Mke.Figure()
	ax = Mke.Axis(fig[1, 1],
	...
	funplot!(fig, ExponentialVariogram())
	fig
end

# ╔═╡ dc35d178-4df7-4c1e-827e-03de67427766
md"""
...
# ╔═╡ 942d1ad2-4de3-4eea-a329-eef250993928
md"""
...
begin
	...
	hs = collect(range(eps(), 25, length=100))
	gs = g.(hs)

	ff = Mke.Figure()
	ax = Mke.Axis(ff[1, 1], xlabel = "h", ylabel = "γ(h)")
	Mke.lines!(ax, hs, gs, color = "teal", label="model")
	Mke.vlines!(ax, [r], linestyle = :dash, color = "grey")
	Mke.text!(ax, r-2, 1-0.1, text = "range")
	Mke.hlines!(ax, [s], linestyle = :dash, color = "brown")
	Mke.text!(ax, 23, s+0.05, text = "sill")
	if n > 0
		Mke.hlines!(ax, [n], linestyle = :dash, color = "orange")
		Mke.text!(ax, 23, n+0.05, text = "nugget")
	end
	Mke.ylims!(ax, 0, 1)
	Mke.axislegend(ax)
	ff
...
# ╔═╡ 58378918-0d36-46eb-aafc-ba9d2b9f4d12
let
	p = GaussianProcess(g)
...
# ╔═╡ c3885d9e-fd00-4a30-96dd-596dbd85599a
md"""
...
# ╔═╡ 14edb5c3-97e3-4cf6-b928-8f68c58308cc
@bind sampled PlutoUI.Button("SAMPLE")

# ╔═╡ d48ed6ac-a8a6-433c-9d37-69e07489fdbb
md"Show answer: $(@bind answer PlutoUI.CheckBox(default=false))"

# ╔═╡ de7f8d87-dbbc-4e26-a5be-4d4afe3c4db1
begin
	sampled
	...
	ss   = rand(0.1:0.1:1.0)
	nn = rand(0.0:0.1:ss)
	γ = Model(range=rr, sill=ss, nugget=nn)
end;

# ╔═╡ f452267b-a5b2-4c62-8e2a-16fee11d6a2c
rand(GaussianProcess(γ), CartesianGrid(600, 300)) |> viewer

# ╔═╡ 6b177ee0-f59f-4981-8c53-8ebaa510cac3
answer ? γ : nothing

# ╔═╡ 92a0fd9d-cf4a-4426-8d5a-2f224d6f72f7
md"""
## Remarks
...
# ╔═╡ 8c122654-9c17-49f1-b287-08f90f7e1afd
subtypes(Variogram)
...
manifest_format = "2.0"
project_hash = "a24257793639fa8ac08e11818af93994029a19b5"
...
git-tree-sha1 = "4edfd3a107bf928057935c677831d7a5d542a69c"
...
git-tree-sha1 = "5a23e79c6b4019f6ef6bdc5346ebfcccb2b11fcc"
...
git-tree-sha1 = "97edf0be142f4f538704d1ecfefa21f90ccd388a"
...
git-tree-sha1 = "663a2cd6d0809574e1a716efb836828881439a33"
...
git-tree-sha1 = "54041ab5eb4ced5d21f6f4b251eac60e8e485a58"
...
git-tree-sha1 = "9273bd07c7f7b3ff9a270ca2733dd3bfe0ed3a61"
...
git-tree-sha1 = "2843a56ee398aa594e111f33c2ff48eb5653842e"
...
git-tree-sha1 = "604078278efdc33f05d45c5613e15f859532e158"
...
git-tree-sha1 = "d32b442995012cb3bc130d995010043c6196112e"
...
git-tree-sha1 = "6d4e71c69298d4f1225a2245fd60530f79e74b31"
...
git-tree-sha1 = "0abe1bf185faae5cbf86a1d59e59552082100c50"
...
deps = ["AbstractTrees", "CategoricalArrays", "CoDa", "ColumnSelectors", "DataScienceTraits", "Distributed", "Distributions", "InverseFunctions", "LinearAlgebra", "NelderMead", "PrettyTables", "Random", "Statistics", "StatsBase", "TableDistances", "Tables", "TransformsBase", "Unitful"]
git-tree-sha1 = "aeeeff24afe2c62ee124f9a101aa81f3e5e572cc"
