...
# ╔═╡ b7cabbe2-917d-11ec-3841-11ec672ac3a5
begin
	# main packages
	using GeoStats
	using GeoTables
...
# ╔═╡ 2c95c31a-576a-404c-8c2c-829ca54e124c
md"""
$\newcommand{\s}{\boldsymbol{s}}\newcommand{\R}{\mathbb{R}}\newcommand{E}{\mathbb{E}}$
"""

# ╔═╡ 8011a99a-90af-4540-8426-087176cae5b1
md"""
# Interpolation in Hilbert spaces

Suppose we are given three functions $f$, $g$ and $h$ at specific locations in the map.

How can we estimate the function at a new location marked with $\times$?

![series](https://i.imgur.com/YKhTqth.png)

This tutorial describes the Hilbert-Kriging framework proposed by [Menafoglio et al. 2013](https://doi.org/10.1214/13-EJS843).
"""

# ╔═╡ 8a2a3a6f-7292-4527-a3ee-83c062e7ad62
md"""
## Introduction

Consider a collection of $k$ georeferenced objects $z(\s_1), z(\s_2), \ldots, z(\s_k) \in \mathcal{H}$ in a Hilbert space (i.e. vector space with inner product) where $\s_1,\s_2,\ldots,\s_k \in D$ are the locations of the objects in a geospatial domain.

For example, consider vectors with $d$ components (i.e. $\mathcal{H}=\R^d$) and their usual inner product

$\langle x, y \rangle = \sum_{i=1}^d x_i y_i,$

or square-integrable functions (i.e. $\mathcal{H}=L^2$) with inner product

$\langle f, g \rangle = \int f(t) g(t) dt.$

Our goal is to define an estimator of the object $z(\s)$ at a new location $\s \in D$ as a weighted combination of the available objects:

$\hat{z}(\s) = \lambda_1\cdot z(\s_1) + \lambda_2\cdot z(\s_2) + \cdots + \lambda_k\cdot z(\s_k)$

Notice that this estimator is written in terms of **scalar multiplication** and **vector addition** in $\mathcal{H}$.

Why do we need the **inner product**?
"""

# ╔═╡ a59cd9af-9c2c-4fc5-b3e7-ce3dabab75e6
md"""
### Hilbert-Kriging

The choice of the inner product $(\langle , \rangle)$ induces a norm ($||\cdot||$), a distance, and consequently a notion of variance in $\mathcal{H}$. This means that the Universal Kriging system of equations for **optimal weights** can be generalized to Hilbert spaces:

$\begin{align}
    \underset{\lambda_1, \lambda_2, \ldots, \lambda_k}{\text{minimize}} \quad & \text{Var}\left(\hat{z}(\s)-z(\s)\right) \\
    \text{subject to} \quad & \E\left[\hat{z}(\s)\right] = m(\s)
\end{align}$
where $m(\s) = \sum_{l=0}^L a_l f_l(\s)$ is the drift term, $f_l$ are pre-specified monomials in $\s$, and $a_l$ are *coefficient objects* in the Hilbert space.

In practice:

1. The coefficient objects in the drift term must be estimated with an implementation of generalized least-squares that supports Hilbert spaces:
$\boldsymbol{\hat{a}} = (\boldsymbol{F}^\top \boldsymbol{\Sigma}^{-1} \boldsymbol{F})^{-1} \boldsymbol{F}^\top \boldsymbol{\Sigma}^{-1} \boldsymbol{z}$
2. The empirical variogram (assuming an intrinsic stationary model) must be estimated in terms of the induced norm in $\mathcal{H}$:
$\hat{\gamma}(\boldsymbol{h}) = \frac{1}{2|N(\boldsymbol{h})|} \sum_{(i,j) \in N(\boldsymbol{h})} ||z(\s_i) - z(\s_j)||^2$

Fortunately, all these details are sorted out in GeoStats.jl.
"""

# ╔═╡ cdd4ea40-217a-46dd-979e-b478e060d422
md"""
## Example: functional data

In order to understand how powerful this idea is, let's define a functional data type that implements the three Hilbert space operations:
"""

# ╔═╡ 0c29ee23-e12d-4655-9cfa-a7d90d2a91b9
begin
	...
	*(λ::Number, f::Fun) = Fun(λ * f.data)
	...
	# inner product
	⋅(f::Fun, g::Fun) = f.data ⋅ g.data

	# displaying functional data
	Base.show(io::IO, f::Fun) =
		show(io, lineplot(f.data, xticks=false, width=70))
end;

# ╔═╡ 46a03278-3548-4ffb-a8b0-bca5fc2b72c1
begin
	# https://github.com/JuliaLang/julia/issues/43903
	import Base: -
	-(f::Fun, g::Fun) = f + (-g)
	-(f::Fun) = -1 * f
end;

# ╔═╡ 23f10df4-3b39-4e0f-9382-abb912b37bb7
md"""
We can use this type to get intuition about weighted combinations of functional data. For example, consider the combination $(1-\lambda)\cdot f + \lambda \cdot g$ of two functions $f, g \in \mathcal{H}$ with $\lambda \in [0,1]$:
"""

# ╔═╡ 8a0f8416-d3fc-45ef-8422-95d656c13136
md"""
λ: $(@bind λ Slider(0:0.1:1, show_value=true))
"""

# ╔═╡ cdb07646-34bf-4935-95c3-3216f575d314
let
...
# ╔═╡ a4cd4810-1587-4f1e-ae25-92ef9865f49c
md"""
The shape of the function $f$ is gradually deformed into the shape of the function $g$ as we increase $\lambda$ from $0$ to $1$. Informally, we say that the scalar multiplication and vector addition determine how objects in a Hilbert space *morph* into one another. 
"""

# ╔═╡ 4939e834-7e60-4d8e-bd36-440c24d73727
md"""
### Meteorological data

Consider time series data from meteorological stations in Brazil. Here, we consider stations that are monitored by the [Instituto Nacional de Meteorologia (INMET)](https://portal.inmet.gov.br). These stations provide real-time (hourly or daily) measurements of various climate variables such as temperature, wind speed, etc.
"""

# ╔═╡ d3e128b0-8613-4664-88fc-4509b99ffe35
stations = let
	# retrieve metadata for all stations
	df = INMET.stations()

	# filter operating stations
	df = filter(row -> row.CD_SITUACAO == "Operante", df)

	# dataframe with name and coordinates
	DataFrame(
		NAME = df.CD_ESTACAO,
		LON  = ustrip.(df.VL_LONGITUDE),
		LAT  = ustrip.(df.VL_LATITUDE)
	)
end

# ╔═╡ 1b2c069e-3616-4891-9023-707780430576
md"""
Because INMET imposes a download quota, we subsample stations at random and only download temperature data from dates $(@bind start DatePicker(default=Date(2022,2,1)))
to $(@bind finish DatePicker(default=Date(2022,2,14))):
"""

# ╔═╡ 0eebafcd-a043-46d9-b19b-a5485c92674b
data = let
	# reproducible example
	Random.seed!(12345)
	
	# subsample stations to avoid reaching quota
	substations = sample(eachrow(stations), 80, replace = false)

	# download temperature time series data
	pairs = map(substations) do station
		series = INMET.series(station.NAME, start, finish)
		station.NAME => series.TEMP_MED
	end

	# subset of stations with valid data
	valid = filter(p -> all(!ismissing, p.second), pairs)

	# valid data as dataframe
	DataFrame(valid)
end

# ╔═╡ 7bedf862-4158-4e1c-b462-ecb596c30bc2
md"""
We convert units from °C to K to work with an absolute scale:
"""

# ╔═╡ 6b3e7a5c-1179-486b-b2bf-17a223c5ff20
series = uconvert.(u"K", data)

# ╔═╡ 9703c939-8231-4431-81ac-b23e0582bc38
md"""
and then convert these series into our functional data type:
"""

# ╔═╡ 2d69e379-cd1f-4138-b6d7-0ad90d07f21d
funs = Fun.(eachcol(series))

# ╔═╡ c8783b0b-e332-4eb7-a5b8-3e97eb980f3d
md"""
Finally, we georeference the functions with the coordinates of the stations:
"""

# ╔═╡ da33a5f2-87ca-4f07-9670-1750d9d21186
fdata = let
	# table with functional data
	table = DataFrame(NAME = names(data), FUN = funs)

	# attach coordinates
	ftable = leftjoin(table, stations, on = :NAME)

	# georeference
	georef(ftable, (:LON,:LAT))
end

# ╔═╡ 86665071-52bd-4b4a-bef2-8c258ef38b66
md"""
Let's visualize the stations in the map:
"""

# ╔═╡ 305b7a33-a428-46cb-a843-a3ed08ebb361
begin
	# retrieve geometries of states in Brazil
	BRA = GeoTables.gadm("BRA", children = true)

	# visualize all states and stations
	viz(domain(BRA), showfacets = true)
	viz!(domain(fdata), color = :red, size = 5)
	Mke.current_figure()
end

# ╔═╡ 0bdf6296-5d65-4ccf-ac5c-0cc6a8ff4b01
md"""
### Variography

We start by computing the empirical variogram of our functional data:
"""

# ╔═╡ e62c62b7-a787-476e-a16f-580e18921eed
g = EmpiricalVariogram(fdata, :FUN, maxlag=15.)

# ╔═╡ 34bf5c55-0920-47c4-a8b9-7b3d64fc27bd
plot(g)

# ╔═╡ bc21897c-7660-4eef-9fe7-8c5e3c8678d5
md"""
Notice how the procedure just works with our custom functional data type, and how the empirical variogram has the correct units of variance in K².

We could have detrended the functional data using generalized least-squares to properly model the residual process, but that is not the goal of this tutorial. We proceed and fit a theoretical model for interpolation:
"""

# ╔═╡ 2955d401-2d5d-4348-ace2-d9866c07a492
γ = fit(GaussianVariogram, g, h -> exp(-h))

# ╔═╡ be37ba17-c43f-41f0-bb0d-14734066fb44
plot(g); plot!(γ, 0, 15.)

# ╔═╡ 8c075ea0-ab7d-4fc2-b949-da47a837ff2a
md"""
### Interpolation

We define our estimation problem for the states of Brazil:
"""

# ╔═╡ dce23417-567b-4874-b3b8-86a8a9066060
problem = EstimationProblem(fdata, domain(BRA), :FUN)

# ╔═╡ 8e12c93d-216e-49ef-80d1-3253ff11bdae
md"""
and solve the problem as usual with the Kriging solver:
"""

# ╔═╡ 37ed52ef-17d2-4b5c-a538-b2cd34575f5f
let
	# define (Hilbert-)Kriging solver
	solver = Kriging(:FUN => (variogram=γ, maxneighbors=10))

	# solve the estimation problem
	solve(problem, solver)
end

# ╔═╡ 2dfb9331-a7c6-48fc-8d4a-902ed77f7cd5
md"""
Unlike multivariate approaches that treat functional data as sequences of correlated variables, Hilbert-Kriging treats functions as objects with a single (exotic) variogram model.
"""

# ╔═╡ f567e649-cd6a-4dad-9ccb-493dd06ea5af
md"""
## Remarks

- GeoStats.jl leverages Julia's powerful dispatch system to generate **high-performance code** for any Hilbert space (e.g. functional data, compositional data).
- Additionally, it automates:
  - **Propagation of units**, including units of variance.
  - **Change of support** with complex geometries (upcoming tutorial).
- Implementing similar features in other programming languages would be **extremely hard**.
- For a more advanced example of Hilbert space, see [CoDa.jl](https://github.com/JuliaEarth/CoDa.jl).
"""
...
git-tree-sha1 = "ad17f55b85ba269de9fa5c514cfa5f809c816373"
...
[[deps.INMET]]
deps = ["DataFrames", "Dates", "HTTP", "JSON", "Printf", "Unitful"]
git-tree-sha1 = "cf01675019ea7b22c77aa7cea4f64ba1cc685623"
uuid = "1fb021e9-9ab7-46c1-8a23-23f3c2783cf7"
version = "0.1.0"
...
git-tree-sha1 = "934037e999685df8792f126a3b69149452d9cf14"
