...
When we start Julia, it starts with a single process. In order to run simulations in parallel, the first thing we need to do is increase the number of processes in the pool *before* loading the environment. The command `addprocs` adds a given number of processes for parallel execution:
"""
...
plot(solution, c=:cividis, size=(900,600))

# ╔═╡ 55aaba80-7174-4035-9921-24271f993166
md"""
## Remarks

- Users can utilize hardware resources such as HPC clusters and the cloud to speed up simulation jobs. In theory, the speedup is upper bounded by the number of processes available. In practice, this translates into generating multiple realizations within the time required for a single one.
- On a personal computer, serial simulation can sometimes outperform parallel simulation.
"""
